...
def Rational_PYTORCH_A_F(x, weight_numerator, weight_denominator, training, device):
    ...
    xps = _get_xps(z, len_num, len_deno).to(device)
    numerator = xps.mul(weight_numerator).sum(1)
    expanded_dw = torch.cat([torch.tensor([1.]).to(device), weight_denominator, \
                             torch.zeros(len_num - len_deno - 1).to(device)])
    denominator = xps.mul(expanded_dw).abs().sum(1)
    return numerator.div(denominator).view(x.shape)


def Rational_PYTORCH_B_F(x, weight_numerator, weight_denominator, training, device):
    # P(X) / Q(X) = a_0 + a_1 * X + ... + a_n * X^n /
...
def Rational_PYTORCH_C_F(x, weight_numerator, weight_denominator, training, device):
    # P(X) / Q(X) = a_0 + a_1 * X + ... + a_n * X^n /
...
def Rational_PYTORCH_D_F(x, weight_numerator, weight_denominator, training, device, random_deviation=0.1):
    ...
    if not training:
        # do not add noise
        return Rational_PYTORCH_B_F(x, weight_numerator, weight_denominator, training, device)
    z = x.view(-1)
    len_num, len_deno = len(weight_numerator), len(weight_denominator)
    # xps = torch.vander(z, max(len_num, len_deno), increasing=True)
    xps = _get_xps(z, len_num, len_deno).to(device)
