    ...
    for name, layer in model.named_children():
        ...
        if len(list(childs)) > 0:
            ...
            for n, l in layer.named_children():
                sequential.add_module(*_convert_layer(n, l, version=rational_version, cuda=rational_cuda))
            converted.add_module(name, sequential)
        else:
            converted.add_module(*_convert_layer(name, layer, version=rational_version, cuda=rational_cuda))
    return converted
        

def _convert_layer(name, layer, version, cuda):
    for activation in activations:
        if isinstance(layer, activation):
            return f'Rational_{name}', Rational(version=version, approx_func=activations[activation], cuda=cuda)
    return name, layer
