...
#define CHECK_INPUT(x) CHECK_CUDA(x); CHECK_CONTIGUOUS(x)
    ...
    at::Tensor pau_cuda_forward_D_5_4(torch::Tensor x, torch::Tensor n, torch::Tensor d);
    std::vector<torch::Tensor> pau_cuda_backward_D_5_4(torch::Tensor grad_output, torch::Tensor x, torch::Tensor n, torch::Tensor d);
        
    
    at::Tensor pau_forward_D_5_4(torch::Tensor x, torch::Tensor n, torch::Tensor d) {
        CHECK_INPUT(x);
        CHECK_INPUT(n);
        CHECK_INPUT(d);
    
        return pau_cuda_forward_D_5_4(x, n, d);
    }
    std::vector<torch::Tensor> pau_backward_D_5_4(torch::Tensor grad_output, torch::Tensor x, torch::Tensor n, torch::Tensor d) {
        CHECK_INPUT(grad_output);
        CHECK_INPUT(x);
        CHECK_INPUT(n);
        CHECK_INPUT(d);
    
        return pau_cuda_backward_D_5_4(grad_output, x, n, d);
    }
